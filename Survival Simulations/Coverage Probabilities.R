...
cov_prob_my<-matrix(NA,nrow=10,ncol=3)
cov_prob_jack<-matrix(NA,nrow=10,ncol=3)
cov_prob_sub<-matrix(NA,nrow=10,ncol=3)
cov_prob_db<-matrix(NA,nrow=10,ncol=3)
...
for(sim_number in 1:3){
  ...
  myVIMP_ind<-matrix(NA,nrow=10,ncol=250)
  jackVIMP_ind<-matrix(NA,nrow=10,ncol=250)
  subVIMP_ind<-matrix(NA,nrow=10,ncol=250)
  dbVIMP_ind<-matrix(NA,nrow=10,ncol=250)
...
colnames(cov_prob_my)<-paste("Sim",1:3,sep="")
rownames(cov_prob_my)<-paste("X",1:10,sep="")
write.csv(cov_prob_my,file="MyCoverage.csv")

colnames(cov_prob_jack)<-paste("Sim",1:3,sep="")
rownames(cov_prob_jack)<-paste("X",1:10,sep="")
write.csv(cov_prob_jack,file="JackknifeCoverage.csv")

colnames(cov_prob_sub)<-paste("Sim",1:3,sep="")
rownames(cov_prob_sub)<-paste("X",1:10,sep="")
write.csv(cov_prob_sub,file="SubsamplingCoverage.csv")

colnames(cov_prob_db)<-paste("Sim",1:3,sep="")
rownames(cov_prob_db)<-paste("X",1:10,sep="")
...
trueVimp_all<-matrix(NA,nrow=10*3,ncol=3)
trueVimp_all[,2]<-paste("Sim",rep(1:3,each=10),sep="")
trueVimp_all[,3]<-paste("X",1:10,sep="")
j<-0
for(i in 1:3){
  load(file=paste("True VIMP\\TrueVIMP_Sim",i,".RData",sep=""))
  trueVimp_all[c(1:10)+j,1]<-trueVIMP
  j<-j+10
}
...
for(i in 1:3){
  ...
  trueVimp_all[c(1:10)+j,"MyCovProb"]<-cov_prob
  j<-j+10
}
...
for(i in 1:3){
  ...
  trueVimp_all[c(1:10)+j,"JackCovProb"]<-cov_prob
  j<-j+10
}
...
for(i in 1:3){
  ...
  trueVimp_all[c(1:10)+j,"SubCovProb"]<-cov_prob
  j<-j+10
}
...
for(i in 1:3){
  ...
  trueVimp_all[c(1:10)+j,"DBCovProb"]<-cov_prob
  j<-j+10
}
