...
setwd("C:/Users/hlcoo/Dropbox/USI/Research/CI for VIMP/Analysis/Examples/Survival")
...
#take out calories at meals due to over 20% missing values
data_use<-data_use[,-9]
...
str(data_use)
data_use$status<-data_use$status-1
data_use$sex<-data_use$sex-1
data_use$status<-factor(data_use$status,levels=c(0,1),labels=c("Censored","Dead"))
data_use$status<-relevel(data_use$status,ref="Dead")
data_use$sex<-factor(data_use$sex,levels=c(0,1),labels=c("Male","Female"))
data_use$ph.ecog<-factor(data_use$ph.ecog,levels=c(0:4),labels=c("Asymp","SympAmb","BedLess50perc","BedOver50perc","Bedbound"))

str(data_use)

cor(data_use[,c(4,7:9)])
...
nodesize<-rep(0,length(ntrees))
oob_tree<-rep(0,length(ntrees))

#loop to find best mtry per level of ntree based on out of bag (oob) error
for(i in 1:length(ntrees)){
  res<-tune(Surv(time=time,event=status,type="right")~.,data=data_use,ntreeTry = ntrees[i])
  res<-data.frame(res$results)
  nodesize[i]<-res[which.min(res$err),1]
  m_try[i]<-res[which.min(res$err),2]
  oob_tree[i]<-res[which.min(res$err),3]
}

#make data frame of tree size, mtry, and error
tune_res<-data.frame(ntrees,m_try,nodesize,oob_tree)
...
#nodesize to use
nodesize_use<-min_oob$nodesize


rf.fit<-rfsrc(Surv(time=time,event=status)~.,data=data_use,mtry = mtry_use,ntree = ntrees_use,importance="permute",
              samptype = "swr",seed=-3,block.size =1,nodesize = nodesize_use)
...
for(i in 1:n_trees){
  ...
  rftree_vimp[,i]<-rftree$importance[,1]
}
...
# rf_samp3<-subsample(rf.fit,B=n_subs,subratio = (1/(nrow(data_use))^(3/4)))
...
# plot.subsample(rf_samp3,main="Subsampling \n95% VIMP CI",alpha=0.05,standardize = F,normal=T,jknife = F, xlab="VIMP")
# plot.subsample(rf_samp3,main="Delete-d Jackknife \n95% VIMP CI",alpha=0.05,standardize = F,normal=T,jknife = T, xlab="VIMP")
