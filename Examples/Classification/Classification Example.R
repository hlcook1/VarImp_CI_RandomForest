...
setwd("C:/Users/hlcoo/Dropbox/USI/Research/CI for VIMP/Analysis/Examples/Classification")
...
data_use$mpg_ind<-as.factor(ifelse(data_use$mpg<21.07,"BelowAverage","AboveAverage"))
str(data_use)
data_use<-data_use[,-1]

table(data_use$mpg_ind)/sum(table(data_use$mpg_ind))*100

dim(data_use)

data_use2<-data_use
data_use2$Cylinders<-as.numeric(data_use2$Cylinders)
data_use2$ModelYear<-as.numeric(data_use2$ModelYear)
str(data_use2)
cor(data_use2[,c(1:6)])
...
for(i in 1:length(ntrees)){
  res<-tuneRF(data_use[,-c(ncol(data_use))],data_use$mpg_ind,ntreeTry = ntrees[i],stepFactor = 2,plot = F,trace = F)
  res<-data.frame(res)
...
rf.fit<-rfsrc(mpg_ind~.,data=data_use,mtry = mtry_use,ntree = ntrees_use,importance="permute",
