...
cov_prob_my<-matrix(NA,nrow=20,ncol=12)
cov_prob_jack<-matrix(NA,nrow=20,ncol=12)
cov_prob_sub<-matrix(NA,nrow=20,ncol=12)
cov_prob_db<-matrix(NA,nrow=20,ncol=12)
...
trueVimp_all<-matrix(NA,nrow=20*12,ncol=3)
trueVimp_all[,2]<-paste("Sim",rep(1:12,each=20),sep="")
...
# library(ggplot2)
# ggplot(trueVimp_long5, aes(x = Quantile, y = Coverage,fill=Method)) +  # ggplot function
#   geom_boxplot()+
#   ylim(0,100)+ 
#   theme_bw()+
#   geom_hline(yintercept = 95,linetype=2)
# 
# stats_all<-boxplot(Coverage~Method*Quantile,data=trueVimp_long5)
# stats_all$stats

png(file="CovProb_all.png",width=1000,height=750)
...
legend("bottomleft",legend=c("Double Bootstrap","Jackknife","Our Bootstrap","Subsampling"),
       pch=15,col=c("blue","green","purple","red"),cex=1.5,bty="n")
legend(5,15,legend=c(paste("mean=",round(mean(cov_prob_db),0),sep=""),
